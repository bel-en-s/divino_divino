  ...
  if (totalPoints > l) {
    ...
    for (ci = 0; ci < l; ci++) {
      ...
      for (j = 1; j <= curvePoints; j++) {
        ...
        a = segment[i];
        points.push(
          _round((t * t * (segment[i + 6] - a) + 3 * inv * (t * (segment[i + 4] - a) + inv * (segment[i + 2] - a))) * t + a),
          // x
          _round((t * t * (segment[i + 7] - (a = segment[i + 1])) + 3 * inv * (t * (segment[i + 5] - a) + inv * (segment[i + 3] - a))) * t + a)
          // y
        );
      }
...
//# sourceMappingURL=chunk-RJRE5L2W.js.map
